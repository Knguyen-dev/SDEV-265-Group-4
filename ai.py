...
# Now read and load in the api key from the txt file for api key, using a temp variable apiKey
apiKey = ""
...
class StoryGPT(ModelBase):
	...
	def sendRemixPrompt(self, story: str, twist: str):
		...
		'''
		self.prompt = f'Remix this story: "{story}".\nThe twist for this remix: {twist}\Write the remix in this style: {self.response_style}'
		self.prompt += self.manager.inject()